...
class Searchable(Indexable):
    ...
    @classmethod
    def search_all(cls, query=None, filters=None, ordering=None):
        """performs a query on self and all subclasses using the model's `.search_objects` manager
        ...
        """
        results = []
        for subclass in cls.get_children():
            qs = list(subclass.search(query, filters, ordering))
            results.extend(qs)
        return results
...
class Aggregateable(Searchable):
    ...
    @classmethod
    def get_all_aggregates(cls, query=None, filters=None):
        """performs an aggregation query on all subclasses using the model's `.search` class method
        ...
        results = {}
        for subclass in cls.get_children():
            aggregates = subclass.get_aggregates(query, filters)
            for dunder_field, keys_counts in aggregates.items():
                results.setdefault(dunder_field, {})
                for key, count in keys_counts.items():
                    results[dunder_field].setdefault(key, 0)
                    results[dunder_field][key] += count
        return results
